...
monitoring_tt_bp = Blueprint('monitoring_tt', __name__)

def load_column_mapping():
    try:
        with open('data/monitoring_TT.json', 'r', encoding='utf-8') as file:
            return json.load(file)
    except (FileNotFoundError, json.JSONDecodeError):
        print("Error: 'data/monitoring_TT.json' file not found or invalid JSON.")
        return {}
    ...
    try:
        ...
        for sheet in workbook.worksheets:
            for row in range(2, sheet.max_row + 1):
                # Отримуємо значення стовпця A
                col_a_value = sheet.cell(row=row, column=1).value

                # Якщо значення в стовпці A порожнє, пропускаємо цей рядок
                if col_a_value is None or str(col_a_value).strip() == '':
                    continue

                # Якщо значення в стовпці A є, додаємо дані
                product_details = {}
                ...
                if not product_id:
                    continue  # Пропускаємо рядки без ID

                # Обчислюємо ціну, якщо потрібно
                try:
                    price_per_kg = float(product_details.get('price_per_kg', 0))
                    product_details['price'] = round(price_per_kg, 2)
                except (ValueError, TypeError):
                    product_details['price'] = 0.0

                # Зберігаємо валідний рядок
...
def compare_product_data(file1, file2):
    ...
    for product_id in all_product_ids:
        ...
        if price1 is not None and price2 is not None:
            ...
            # Відмітка рядків із значними відхиленнями
        ...
        # Додаємо всі рядки до результатів
...
@monitoring_tt_bp.route('/result', methods=['GET', 'POST'])
def monitoring_TT():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('monitoring_TT/iteration.html', results=significant_results, summary=summary)
        except Exception as e:
            print(f"Error: {e}")
            return render_template('error.html', error="File processing error. Please try again.")

    return render_template('monitoring_TT/index.html')

@monitoring_tt_bp.route('/edit_prices_TT', methods=['POST'])
def edit_prices_TT():
    global last_results, last_summary
...
@monitoring_tt_bp.route('/export_excel_TT')
def export_excel_TT():
    if last_results is None:
...
@monitoring_tt_bp.route('/export_csv_TT')
def export_csv_TT():
    if last_results is None:
        abort(400, "No data available for export")
